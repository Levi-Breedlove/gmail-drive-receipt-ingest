 ...
 * - Saves a self-contained HTML body snapshot
 * - Saves a PDF converted from that HTML snapshot
 * - Saves the raw original email as .eml
 * - Saves regular attachments separately
 * - Embeds cid:inline images into the HTML snapshot for better fidelity
 * - Keeps idempotency, logs, labels, and reset helper
 *
 * Required services:
 ...
 *
 * Notes:
 * - HTML is the highest-fidelity artifact in this Apps Script-only design.
 * - PDF is a convenience output created from the HTML blob.
 * - True 1:1 PNG screenshot capture is not supported in Apps Script-only server-side automation.
 */
...
function processLabel_(labelName, config, summary) {
  ...
  threads.forEach(thread => {
    ...
    try {
      ...
      markProcessed_(thread, messageId, config);
...
function saveEmailPackage_(message, apiBundle, context, config) {
  ...
  const renderHtml = buildRenderableEmailHtml_(message, apiBundle.parsed);

  // Save only the email body PDF
  if (config.savePdf) {
  ...
  // Save normal attachments separately
...
function buildRenderableEmailHtml_(message, parsed) {
  ...
  // First embed cid: inline images gathered from MIME parts.
  html = inlineCidImages_(html, parsed.inlineImages);

  // Then fetch remote/proxied images and inline them too.
...
function fetchImageDataUris_(urls) {
  ...
  try {
    ...
    Logger.log(`WARN fetchAll failed for remote images: ${err.message}`);
    return map;
  }
  ...
  responses.forEach((response, index) => {
    ...
    try {
      ...
      Logger.log(`WARN failed processing remote image ${url}: ${err.message}`);
    }
  });
...
<html>
  ...
  // Has head/body tags but not wrapped in <html>.
...
function decodeBase64UrlToBytes_(data) {
  ...
  // Case 1: already a byte array / numeric array-like object
  if (Array.isArray(data)) {
    return data;
  }

  // Some Apps Script advanced-service responses behave like array-ish objects
  // rather than plain JS arrays.
  ...
  // Case 2: string that is actually comma-separated bytes already
  ...
  // Case 3: actual base64url/base64 string
...
function markProcessed_(thread, messageId, config) {
...
function markMessageProcessed_(messageId) {
  ...
  props.setProperty('PROCESSED_MESSAGE_IDS', JSON.stringify(map));
}
...
function detectVendor_(message, parsed) {
  ...
  // 3) Search across actual email content
...
function extractForwardedFromDisplayName_(plainBody) {
  ...
  // Ignore your known forwarding sender
...
function resetTestProcessedState() {
  ...
  Logger.log('Removed processed/review labels and cleared PROCESSED_MESSAGE_IDS.');
}
